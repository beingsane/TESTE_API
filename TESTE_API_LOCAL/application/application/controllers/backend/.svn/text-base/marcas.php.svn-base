...
class Marcas extends CI_Controller_Auth_Permition {
	public function index()	{
		...
		/////Carrega os compras cadastrados
		$data_de	= $this->input->post("data_inicial");
		$data_ate	= $this->input->post("data_final");
		$this->load->model('backend/compras/compras_model', 'load_compras');
		$data['compras'] = $this->load_compras->lista(null,$data_de, $data_ate);
		
		/////Monta a tollbar
		$this->tollbar->setTextTollbar('glyphicons money','Gerenciamento de compras : Lista!');
		$this->tollbar->setTollbar("Nova Compra",	"circle_plus",	"backend","compras",	"add_form");
		$this->tollbar->setTollbar("Filtrar",		"refresh",		"backend","compras",	"index");
		
		/////Carrega a view
		$this->template->load('template/backEndTemplate','backend/compras/index',$data);
	}
	...
	public function add_commit()	{
		...
		if((!$data_separated[0] || !$data_separated[1] || !$data_separated[2]) 
			...
			!checkdate ($data_separated[1], $data_separated[0], $data_separated[2])){
				$error[]	= "";	
				$this->template_functions->setError("info","Preencha uma data de compra válida","Alerta");
		}
	...
	public function edit_commit()	{
		...
		if(!is_numeric($valor) || $valor<0)	{$error[] = "";	$this->template_functions->setError("info","Preencha o valor unitário","Alerta");}
		$data_separated							= explode("/", $data_compra);
		if((!$data_separated[0] || !$data_separated[1] || !$data_separated[2]) 
				||
			!checkdate ($data_separated[1], $data_separated[0], $data_separated[2])){
